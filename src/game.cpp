...
void Game::renderGame()
{
	...
	if (SUCCEEDED(graphics->beginScene()))
	{
		...
		if (fpsOn)           // if fps display requested
		{
			...
			dxFont.print(buffer, GAME_WIDTH - 120, GAME_HEIGHT - 30);
		}

		if (playerHitOn)           // if fps display requested
		{
			// convert fps to Cstring
			_snprintf_s(buffer, BUF_SIZE, "Player Hit ", (int)playerHit);
			dxFont.print(buffer, GAME_WIDTH - 200, GAME_HEIGHT / 2);
		}
